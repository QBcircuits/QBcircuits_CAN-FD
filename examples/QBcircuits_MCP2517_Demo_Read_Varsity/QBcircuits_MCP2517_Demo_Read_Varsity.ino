...
void setup() {
  ...
  if(can1_bL){                                                                                  // CAN FD chn1 is enabled
    ...
    rVal[1] = mcp2517_fltr_setup(&can1,FIFO1,FLTRNUM0,FLTRIDX0,FLTRBOTH,0x000,0x000);           // setup CAN FD chn 1 Rx filter (*ptrChn,bufIdx,fltrNum,fltrIdx,fltrType,msgId,mskId)
  }
  if(can2_bL){                                                                                  // CAN FD chn1 is enabled
    ...
    rVal[3] = mcp2517_fltr_setup(&can2,FIFO1,FLTRNUM0,FLTRIDX0,FLTRBOTH,0x000,0x000);           // setup CAN FD chn 2 Rx filter (*ptrChn,bufIdx,fltrNum,fltrIdx,fltrType,msgId,mskId)
  }
  if(!rVal[0] && !rVal[1] && !rVal[2] && !rVal[3] && (can1_bL || can2_bL)){                     // check that setup functions were successfull & at least one channel is enabled
    ...
    Serial.print("Channel,Time(s) abs,#of timer overflow,Timer[#of (1/40MHz) periods],ID,IDE,FDF,BRS,RTR,DLC,Length,Data");     // format/print .csv header
    Serial.println();                                                                           // format/print new line
  }
...
void loop(){                                                                                        // system main
  ...
  if(mode_u8){                                                                                      // stateX [start logging & wait for user to stop logging]
    for(j_u8=min_u8;j_u8<max_u8;j_u8++){                                                            // loop thru all chnCAN pointers
      if(mcp2517_check_message(ptrChn[j_u8])){                                                      // check if current chnCAN has recieved a msg
        
        mcp2517_read_register(ADDR_C1INT,ptrChn[j_u8],0);                                           // read MCP2517 C1INT register (byte 0) for TBCIF
        if((ptrChn[j_u8]->regRd[0] >> TBCIF) & 1){                                                  // check if TBCIF is set (indicates if MCP2517 timer has overflowed)
          ...
          mcp2517_write_register(ADDR_C1INT,ptrChn[j_u8],0);                                        // write MCP2517 C1INT register (byte 0) to clear TBCIF
        }
        for(i_u8=0;i_u8<CSCNT;i_u8++);                                                              // wait some time before toggling MCP2517 chip select
        mcp2517_read_memory(FIFO1,ptrChn[j_u8]);                                                    // read current chnCAN message stored in FIFO1 
        ...
        Serial.print(mcp2517_id_calc(ptrChn[j_u8]),HEX);  Serial.print(",");                        // format/print ID field
