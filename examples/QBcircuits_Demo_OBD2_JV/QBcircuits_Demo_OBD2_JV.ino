...
void loop(){                                                                                    // system main
  ...
  mcp251xfd_msg_write(&can1,0x7E0,!IDE,!FDF,!BRS,!RTR,8,txData);                                // populate channel struct with necessary transmit info
  ...
  if(mcp251xfd_check_message(&can1)){                                                           // check if current chnCAN has recieved a msg
    mcp251xfd_read_memory(FIFO1,&can1);                                                         // read current chnCAN message stored in FIFO1
