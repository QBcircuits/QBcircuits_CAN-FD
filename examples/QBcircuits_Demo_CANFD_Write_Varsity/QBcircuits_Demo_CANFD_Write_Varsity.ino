...
void setup() {
  ...
  if(can1_bL){                                                                                  // CAN FD chn1 is enabled
    ...
    rVal[1] = mcp251xfd_fltr_setup(&can1,FIFO1,FLTRNUM0,FLTRIDX0,FLTREXID,0x000,0xFFFFFFF);     // setup CAN FD chn 1 Rx filter to only recieve standard or extended msg ID 0x000
  }
  if(can2_bL){                                                                                  // CAN FD chn1 is enabled
    ...
    rVal[3] = mcp251xfd_fltr_setup(&can2,FIFO1,FLTRNUM0,FLTRIDX0,FLTREXID,0x000,0xFFFFFFF);     // setup CAN FD chn 2 Rx filter to only recieve standard or extended msg ID 0x000
  }
...
void loop(){                                                                                    // system main
  ...
  if(mode_u8 == 0){                                                                             // state0 [Prompt user of available options]
    ...
    for(j_u8=0;j_u8<MSGMAX;j_u8++){                                                             // loop thru all defined CANbus messages
        ...
        bfDlc = mcp251xfd_dlc_payload(bfFdf,pLen);                                              // calculate and store current msg DLC value
  ...
  // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if((can1_bL && chnIdx==CAN1) || (can2_bL && chnIdx==CAN2)){                                 // check that channel index for the chosen message is enabled
      ...
      mcp251xfd_msg_write(ptrChn[chnIdx],msgId[idx],bfIde,bfFdf,bfBrs,!RTR,pLen,txData);        // populate channel struct with necessary transmit info
...
//    mode_u8 = 4;                                                                                // uncomment this line to go to debug state
  ...
  // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if(mcp251xfd_check_message(ptrChn[!chnIdx])){                                               // check if opposite CAN channel has recieved a msg
      mcp251xfd_read_memory(FIFO1,ptrChn[!chnIdx]);                                             // read the msg recieved by the opposite CAN channel 
      Serial.print("Recieved message: 0x");                                                     // format/print label
      Serial.println(mcp251xfd_id_calc(ptrChn[!chnIdx]),HEX);                                   // format/print ID field
    }
