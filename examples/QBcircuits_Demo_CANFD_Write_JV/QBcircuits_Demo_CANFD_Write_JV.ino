...
unsigned long msgId[MSGMAX] = {0x111,0x433,0x555,0x777,0x0,0x0};
...
void setup() {
  ...
  if(can1_bL){                                                                                  // CAN FD chn1 is enabled
    ...
    rVal[1] = mcp251xfd_fltr_setup(&can1,FIFO1,FLTRNUM0,FLTRIDX0,FLTRBOTH,0x000,0xFFFFFFF);     // setup CAN FD chn 1 Rx filter to only recieve standard or extended msg ID 0x000
  }
  if(can2_bL){                                                                                  // CAN FD chn1 is enabled
    ...
    rVal[3] = mcp251xfd_fltr_setup(&can2,FIFO1,FLTRNUM0,FLTRIDX0,FLTRBOTH,0x000,0xFFFFFFF);     // setup CAN FD chn 2 Rx filter to only recieve standard or extended msg ID 0x000
  }
...
void loop(){                                                                                    // system main
  ...
  for(i_u8=0;i_u8<64;i_u8++){txData[i_u8]=i_u8;}												                        // set each value in buffer to index value
  // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  mcp251xfd_msg_write(&can1,msgId[j_u8++],bfIde,bfFdf,bfBrs,!RTR,pLen,txData);                  // populate channel struct with necessary transmit info
  mcp251xfd_write_memory(TXQ,&can1);                                                            // write the Tx message info to MCP2517 TXQ
  mcp251xfd_start_transmit(TXQ,&can1);                                                          // initiate a transmit onto the CANbus network
  delay(20);                                                                                    // wait for message to transmit & screen updating
  if(DBG_BL){                                                                                   // debug flag set to 1
    if(mcp251xfd_check_message(&can2)){                                                         // check if current chnCAN has recieved a msg
      mcp251xfd_read_memory(FIFO1,&can2);                                                       // read current chnCAN message stored in FIFO1
      Serial.print("Recieved message ID: 0x");                                                  // format/print timestamp
      Serial.println(mcp251xfd_id_calc(&can2),HEX);                                             // format/print ID field
    }
  }
  // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
